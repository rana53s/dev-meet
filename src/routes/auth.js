...
const { validateSignUpData, validateIfPasswordIsStrong } = require('../utils/validation');
...
authRouter.post('/signup', async(req, res) => {
    ...
    try {
        ...
        const { firstName, lastName, emailId, age, gender, password, imageUrl, skills } = req.body;
...
authRouter.post('/login', async(req, res) => {
    ...
    try {
        ...
        if (!user) {
            throw new Error ("No user is found with the credentials, please enter valid ones")
        }
        ...
        if (isPasswordValid) {
            ...
            res.send({
                name: user.firstName + " signed in successfully!",
                jwtToken: token
            });
        } else {
            throw new Error("No user is found with the credentials, please enter valid ones");
        ...
        res.status(400).send("Error while signing you in :- " + err.message);
    }
});
